...
describe('SeoService', () => {
  ...
  it('should update title with app name prefix when a key is provided', async() => {
    ...
    // --- ASSERT ---
    const EXPECTED_TITLE = 'Cofrap | My Page';
  ...
  it('should use only app name if titleKey matches default title (Home case)', async() => {
    ...
    // --- ASSERT ---
    expect(TITLE_MOCK.setTitle).toHaveBeenCalledWith('Cofrap');
    expect(META_MOCK.updateTag).toHaveBeenCalledWith({ property: 'og:title', content: 'Cofrap' });
  });
